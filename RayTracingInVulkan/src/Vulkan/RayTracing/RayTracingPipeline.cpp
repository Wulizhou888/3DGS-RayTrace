	...
	swapChain_(swapChain)
{
	...
	const std::vector<DescriptorBinding> descriptorBindings =
	{
		...
		// WLZ: allow Intersectioin Shader to access
		{3, 1, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR | VK_SHADER_STAGE_INTERSECTION_BIT_KHR},
		...
		// WLZ: Gauss Procedural buffer
		{12, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_INTERSECTION_BIT_KHR},

		// WLZ: Next-K-Splats buffer
		{13, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_INTERSECTION_BIT_KHR},

		// WLZ: Ray Info
		{14, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_INTERSECTION_BIT_KHR},

		// WLZ: Exp LUT
		{15, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_INTERSECTION_BIT_KHR},
	...
	for (uint32_t i = 0; i != swapChain.Images().size(); ++i)
	{
		...
		// WLZ: Procedural Gauss buffer (Intersection Shader) & NextK buffer (Anyhit Shader)
		VkDescriptorBufferInfo proceduralGaussBufferInfo = {};
		VkDescriptorBufferInfo nextKGaussBufferInfo = {};
		VkDescriptorBufferInfo rayBufferInfo = {};
		VkDescriptorBufferInfo expLUTBufferInfo = {};

		if (scene.HasProceduralGauss())
		{
			proceduralGaussBufferInfo.buffer = scene.ProceduralGaussBuffer().Handle();
			proceduralGaussBufferInfo.range = VK_WHOLE_SIZE;
			nextKGaussBufferInfo.buffer = scene.NextKGaussBuffer().Handle();
			nextKGaussBufferInfo.range = VK_WHOLE_SIZE;
			rayBufferInfo.buffer = scene.RayInfoBuffer().Handle();
			rayBufferInfo.range = VK_WHOLE_SIZE;
			expLUTBufferInfo.buffer = scene.ExpLUTBuffer().Handle();
			expLUTBufferInfo.range = VK_WHOLE_SIZE;

			descriptorWrites.push_back(descriptorSets.Bind(i, 12, proceduralGaussBufferInfo));
			descriptorWrites.push_back(descriptorSets.Bind(i, 13, nextKGaussBufferInfo));
			descriptorWrites.push_back(descriptorSets.Bind(i, 14, rayBufferInfo));
			descriptorWrites.push_back(descriptorSets.Bind(i, 15, expLUTBufferInfo));
		}
	...
	switch (shaderType) {
		...
		case 5:
			...
			break;
		case 6:  // WLZ
			printf("RTV: Using Gauss Ray Tracing. -by WLZ\n");
			rayGenShader = new ShaderModule(device, "../assets/shaders/GaussTracing.rgen.spv");
			closestHitShader = new ShaderModule(device, "../assets/shaders/RayTracing.rchit.spv");
			// closestHitShader = new ShaderModule(device, "../assets/shaders/RayTracing.ProceduralGauss.rchit.spv");
			break;
		default:
	...
	const ShaderModule proceduralGaussClosestHitShader(device, "../assets/shaders/RayTracing.ProceduralGauss.rchit.spv"); // WLZ
	const ShaderModule proceduralGaussIntersectionShader(device, "../assets/shaders/RayTracing.ProceduralGauss.rint.spv"); // WLZ
	...
	std::vector<VkPipelineShaderStageCreateInfo> shaderStages =
	{
		...
		proceduralGaussClosestHitShader.CreateShaderStage(VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR), // WLZ
		...
		proceduralGaussIntersectionShader.CreateShaderStage(VK_SHADER_STAGE_INTERSECTION_BIT_KHR) // WLZ
		#endif
	};
	...
	proceduralCubeHitGroupInfo.intersectionShader = 8;
	...
	proceduralCylinderHitGroupInfo.intersectionShader = 9;
	proceduralCylinderHitGroupIndex_ = 5;

	// WLZ
	VkRayTracingShaderGroupCreateInfoKHR proceduralGaussHitGroupInfo = {};
	proceduralGaussHitGroupInfo.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
	proceduralGaussHitGroupInfo.pNext = nullptr;
	proceduralGaussHitGroupInfo.type = VK_RAY_TRACING_SHADER_GROUP_TYPE_PROCEDURAL_HIT_GROUP_KHR;
	proceduralGaussHitGroupInfo.generalShader = VK_SHADER_UNUSED_KHR;
	proceduralGaussHitGroupInfo.closestHitShader = 6;
	proceduralGaussHitGroupInfo.anyHitShader = VK_SHADER_UNUSED_KHR;
	proceduralGaussHitGroupInfo.intersectionShader = 10;
	proceduralGaussHitGroupIndex_ = 6;
	...
	std::vector<VkRayTracingShaderGroupCreateInfoKHR> groups =
	{
		...
		proceduralGaussHitGroupInfo // WLZ
