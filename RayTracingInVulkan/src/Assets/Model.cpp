...
// WLZ
Model Model::CreateGauss(const vec3& center, const vec4 &rot, const vec3 &scale, const float opacity, const Material& material, const bool isProcedural)
{
	std::vector<Vertex> vertices;
	std::vector<uint32_t> indices;
	...
	return Model(
		...
		nullptr,
		nullptr,
		isProcedural ? new Gauss(center, rot, scale, opacity) : nullptr);
}
...
// WLZ
Model::Model(std::vector<Vertex>&& vertices, std::vector<uint32_t>&& indices, std::vector<Material>&& materials, const class Procedural* procedural, const class Procedural* proceduralCube, const class Procedural* proceduralCylinder, const class Procedural* proceduralMandelbulb, const class Procedural* proceduralGauss) :
	vertices_(std::move(vertices)),
	indices_(std::move(indices)),
	materials_(std::move(materials)),
	procedural_(procedural),
	...
	proceduralCylinder_(proceduralCylinder),
	proceduralMandelbulb_(proceduralMandelbulb),
	proceduralGauss_(proceduralGauss)
{
}
