...
const std::vector<std::pair<std::string, std::function<SceneAssets (SceneList::CameraInitialSate&)>>> SceneList::AllScenes =
{
	...
	{"3DGS", GaussSplat}  // 3DGS
};


// WLZ
SceneAssets SceneList::GaussSplat(CameraInitialSate& camera)
{
	...
	models.push_back(Model::CreateSphere(vec3(200, 200, 0), 0.5, Material::Metallic(vec3(0.7f, 0.5f, 0.8f), 0.2f), false));
	models.push_back(Model::CreateGauss(vec3(0, 0, 5), vec4(1, 0, 0, 0), vec3(1, 1, 1), 0.9, Material::Dielectric(1.5f), true));
	models.push_back(Model::CreateGauss(vec3(0, 0, 3), vec4(1, 0, 0, 0), vec3(2, 2, 2), 0.9, Material::Dielectric(1.5f), true));
